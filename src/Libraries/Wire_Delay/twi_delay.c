...
// Доступ к задержкам из класса TwoWire
extern uint8_t delayAfterAddress;
extern uint8_t delayAfterData;
extern uint8_t delayAfterRead;
extern uint8_t delayAfterStop;
...
//Функция задержки
static inline void apply_delay(uint8_t delay) {
  if (delay) {
    sei();
...
ISR(TWI_vect)
{
  switch(TW_STATUS){
    ...
    case TW_MT_SLA_ACK: // slave receiver acked address
		apply_delay(delayAfterAddress); // 50 мкс после адреса
		if(twi_masterBufferIndex < twi_masterBufferLength){
			...
			if (twi_sendStop){																   
				twi_stop();
				apply_delay(delayAfterStop); // 50 мкс после STOP
			} else {
				twi_inRepStart = true;
    ...
    case TW_MT_DATA_ACK: // slave receiver acked data
	  ...
	  //Задержка 1 мС между байтами
	  apply_delay(delayAfterData);
      if(twi_masterBufferIndex < twi_masterBufferLength){
        ...
        if (twi_sendStop){
          twi_stop();
		  //Задержка 50 мкС после STOP
		apply_delay(delayAfterStop);
       } else {
    ...
    case TW_MR_DATA_NACK: // data received, nack sent
      ...
      twi_masterBuffer[twi_masterBufferIndex++] = TWDR;
		//Задержка 1 мс после последнего байта
	  apply_delay(delayAfterRead);						  
      if (twi_sendStop){
        twi_stop();
		//Задержка 50 мкс после последнего STOP
	  apply_delay(delayAfterStop);
      } else {
        twi_inRepStart = true;	// we're gonna send the START
        // don't enable the interrupt. We'll generate the start, but we
